...
JoystickState::JoystickState(TalonXXIII* pRobot)
{
    mainRobot = pRobot;
...
void JoystickState::SetGamepadAxisParameter(int axisNum, double deadband, double shaping)
{
    if ((axisNum < 0) || (axisNum >= MAX_JOYSTICK_AXES))
    {
...
void JoystickState::SetFlightControllerAxisParameter(int axisNum, double deadband, double shaping)
{
    if ((axisNum < 0) || (axisNum >= MAX_JOYSTICK_AXES))
    {
...
void JoystickState::UpdateDash()
{
    ...
    frc::SmartDashboard::PutNumber("rawrotating", flightcontroller->GetRawAxis(5));
...
double JoystickState::GetGamepadAxis(int axisNumber)
{
...
double JoystickState::GetFlightControllerAxis(int axisNumber)
{
...
bool JoystickState::GetDpadPushed()
{
    if(GetDpadLeftPushed() || GetDpadDownPushed() || GetDpadRightPushed() || GetDpadUpPushed())
        return true;
    else
        return false;
}
...
double JoystickState::AxisShaping(double rawReading, double deadband, double shapingSlope)
{
    double modifiedReading;
...
{
    if (gamepadBtnCount == 0 || flightcontrollerBtnCount == 0)
    {
        JoystickCountInitialize();
    }     
